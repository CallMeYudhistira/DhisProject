...
@section('content')
                    ...
                    <div class="row align-items-center {{ $loop->even ? 'flex-row-reverse' : '' }} g-4 g-lg-5">
                        <div class="col-12 col-lg-6">
                            <div class="shadow-lg rounded-4 overflow-hidden transition-hover" style="aspect-ratio: 16 / 9;">
                                @if ($project->preview)
                                    <img src="{{ url($project->preview) }}" class="w-100 h-100" alt="{{ $project->title }}" style="object-fit: cover;">
                                @else
                                    <div class="bg-secondary d-flex align-items-center justify-content-center text-white w-100 h-100">
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="project-info {{ $loop->even ? 'pe-lg-5' : 'ps-lg-5' }} text-center text-lg-start">
                                <span class="badge bg-dark mb-3 px-3 py-2 rounded-pill">0{{ $loop->iteration }} / Project</span>
                                <h2 class="display-4 fw-bold mb-4">{{ $project->title }}</h2>
                                <p class="lead text-muted mb-4" style="line-height: 1.8;">
                                    {{ $project->description }}
                                </p>
                                @if ($project->url)
                                    <a class="btn btn-outline-dark btn-lg rounded-pill px-4" href="{{ $project->url }}" target="_blank">
    ...
    <script>
        ...
        function typeEffect() {
            ...
            if (!isDeleting) {
                typedText.textContent = currentText.substring(0, charIndex + 1); charIndex++;
